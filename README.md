...
HyperFlow is a self-distillation: the base model is the only teacher. In our internal comparisons against the base
pipeline at 49 NFE, on the same seeds, four things stood out:
...
Judge on your own prompts: run the same command once as is and once with `--baseline --output base.mp4`; with the
same `--seed` the two clips start from the same noise.
...
- **Two-time conditioning.** The LoRA was trained with both the current time `t` and the step's endpoint `r` (a
  flow-map formulation). `TwoTimeEmbedder` wraps the base `time_embedder`, adds a LoRA'd copy for `r`,
